...
public class DalamudUtilService : MediatorSubscriberBase, IHostedService
{
            ...
            if ((nint)drawObj != IntPtr.Zero)
            {
                ...
                if (!isDrawing)
                {
                    ...
                    if (!isDrawing)
                    {
                        ...
                        if (isDrawing)
                        {
                            if (!string.Equals(_lastGlobalBlockPlayer, playerInfo.Name, StringComparison.Ordinal) && !string.Equals(_lastGlobalBlockReason, "HasModelFilesInSlotLoaded"))
                            {
                    ...
                    {
                        if (!string.Equals(_lastGlobalBlockPlayer, playerInfo.Name, StringComparison.Ordinal) && !string.Equals(_lastGlobalBlockReason, "HasModelInSlotLoaded"))
                        {
                            _lastGlobalBlockPlayer = playerInfo.Name;
                ...
                {
                    if (!string.Equals(_lastGlobalBlockPlayer, playerInfo.Name, StringComparison.Ordinal) && !string.Equals(_lastGlobalBlockReason, "RenderFlags"))
                    {
                        _lastGlobalBlockPlayer = playerInfo.Name;
    ...
    private unsafe void FrameworkOnUpdateInternal()
    {
        ...
        _performanceCollector.LogPerformance(this, "ObjTableToCharas",
            () => {
                foreach (var gobj in _objectTable)
                {
                    var p = gobj as PlayerCharacter;
                    if (p)
                    {
                        CheckCharacterForDrawing(p);
                        var info = GetPlayerInfo(p);
                        _playerCharas[info.Hash] = (info.Name, p.Address);
                    }
                }
            }
        );
